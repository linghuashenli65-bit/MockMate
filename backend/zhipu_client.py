...
from .config import ZHIPU_API_BASE, ZHIPU_API_KEY, ZHIPU_MODEL, ZHIPU_MODEL_REASONER, ZHIPU_MODEL_CHAT, ZHIPU_MODEL_WRITTEN_EVAL, ZHIPU_MODEL_TTS, ZHIPU_MODEL_VISION
...
class ZhipuClient:
    def __init__(self, api_key: Optional[str] = None,
        ...
        self.reasoner_model = reasoner_model or ZHIPU_MODEL_REASONER
        self.chat_model = chat_model or ZHIPU_MODEL_CHAT
        self.written_eval_model = written_eval_model or ZHIPU_MODEL_WRITTEN_EVAL
        self.tts_model = tts_model or ZHIPU_MODEL_TTS
        self.vision_model = vision_model or ZHIPU_MODEL_VISION
    ...
    def with_key(self, api_key: Optional[str] = None,
                 ...
                 written_eval_model: Optional[str] = None,
                 tts_model: Optional[str] = None,
                 vision_model: Optional[str] = None) -> 'ZhipuClient':
        ...
        client.vision_model = vision_model if vision_model is not None else self.vision_model
        return client
    ...
    async def chat(self, model: str, messages: list, **kwargs) -> Optional[str]:
        ...
        try:
            ...
            if content is None:
                logger.warning(f"Zhipu {model} 返回空内容")
            return content
        except Exception as e:
            logger.error(f"Zhipu chat error ({model}): {e}")
            return None
    ...
    async def reason(self, messages: list, **kwargs) -> Optional[str]:
        ...
        if result is None and self.ready:
            logger.warning("zhipu reasoner 返回空，降级到 chat 模型")
    ...
    async def extract_text_from_image(self, image_bytes: bytes) -> Optional[str]:
        ...
        try:
            resp = await self._client.post(
                "/chat/completions",
                headers=self._headers(),
                json={"model": self.vision_model, "messages": messages, "max_tokens": 2048},
            )
            resp.raise_for_status()
            data = resp.json()
